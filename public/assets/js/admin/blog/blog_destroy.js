$(document).ready(function() {
    ...
    function getItemName($deleteBtn) {
        ...
        if ($tableRow.length > 0) {
            // Desktop table view - get text from first td, remove <strong> tags
            return $tableRow.find('td:first strong').text().trim() || $tableRow.find('td:first').text().trim();
    ...
    function getConfirmMessage(itemName) {
        console.log('Window translations:', window.blogTranslations); // Debug log
        console.log('Item name:', itemName); // Debug log
        
        // Try to get localized message with item name from window translations
        if (window.blogTranslations && window.blogTranslations.confirm_delete_item) {
            var message = window.blogTranslations.confirm_delete_item.replace(':name', itemName);
        ...
        if (window.blogTranslations && window.blogTranslations.confirm_delete) {
            var message = window.blogTranslations.confirm_delete;
     ...
     */
    function deleteBlogItem(itemId, $deleteBtn) {
        // Show loading state
        ...
        // Get current locale from URL or set default
        var currentLocale = getLocaleFromUrl() || 'en';
        
        $.ajax({
            url: '/admin/blogs/delete/' + itemId + '?locale=' + currentLocale,
            type: 'DELETE',
            headers: {
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showAlert(response.message || getLocalizedMessage('delete_success'), 'success');
                    ...
                    // Show error message
                    showAlert(response.message || getLocalizedMessage('delete_error'), 'error');
            ...
            error: function(xhr, status, error) {
                ...
                if (xhr.responseJSON && xhr.responseJSON.message) {
                    ...
                    errorMessage = 'Blog item not found.';
    ...
    function getLocaleFromUrl() {
        var urlParams = new URLSearchParams($(location).attr('search'));
    ...
    function getLocalizedMessage(key) {
        var messages = {
            'delete_success': 'Blog item deleted successfully!',
            'delete_error': 'Error occurred while deleting blog item!',
            'no_data': 'No data available!'
        };
        
        // Try to get from window translations first
        if (window.blogTranslations && window.blogTranslations[key]) {
            return window.blogTranslations[key];
        }
