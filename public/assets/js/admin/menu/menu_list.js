$(document).ready(function() {
    ...
    /**
     * Initialize menu list functionality
     */
    function initializeMenuList() {
        // handleDeleteButtons(); // Disabled - now handled by menu_destroy.js
        handleAddMenuButton();
    ...
    function handleAddMenuButton() {
        $('#addMenuItemBtn').on('click', function(e) {
    ...
    function handleResponsiveAdjustments() {
        ...
        if (windowWidth < 576) {
            $('.menu-item-card .btn span').hide();
        } else if (windowWidth >= 576 && windowWidth < 992) {
            $('.menu-item-card .btn span').show();
        }
    }
    ...
    $(document).on('click', '.btn-info, .btn-warning', function(e) {
        ...
        var $btn = $(this);
        var itemId = $btn.closest('tr, .menu-item-card').find('.delete-btn').data('id');
        ...
        if (action === 'detail') {
            window.location.href = '/admin/menu/detail/' + itemId;
        } else {
            // For edit action
