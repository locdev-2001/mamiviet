$(document).ready(function() {
    ...
    function getItemName($deleteBtn) {
        ...
        var $card = $deleteBtn.closest('.coupon-item-card');
    ...
    function getConfirmMessage(itemName) {
        console.log('Window translations:', window.couponTranslations); // Debug log
        console.log('Item name:', itemName); // Debug log
        
        // Try to get localized message with item name from window translations
        if (window.couponTranslations && window.couponTranslations.confirm_delete_item) {
            var message = window.couponTranslations.confirm_delete_item.replace(':name', itemName);
        ...
        if (window.couponTranslations && window.couponTranslations.confirm_delete) {
            var message = window.couponTranslations.confirm_delete;
    ...
    /**
     * Delete coupon item via AJAX - Using API endpoint
     */
    function deleteCouponItem(itemId, $deleteBtn) {
        // Show loading state
        ...
        // Use the web route for deleting coupon (MVC approach)
        $.ajax({
            url: '/admin/coupons/delete/' + itemId,
            type: 'DELETE',
            headers: {
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    var $item = $deleteBtn.closest('tr, .coupon-item-card');
    ...
    /**
     * Get authentication token (not needed for web routes)
     * This function is kept for compatibility but not used
     */
    function getAuthToken() {
        // Not needed for web routes as they use session-based authentication
        return '';
    }
    ...
    function getLocalizedMessage(key) {
        var messages = {
            ...
            'delete_error': 'Error occurred while deleting coupon!',
            'no_data': 'No data available!'
        };
        
        // Try to get from window translations first
        if (window.couponTranslations && window.couponTranslations[key]) {
            return window.couponTranslations[key];
        }
